...
`agent-plugins` is the package source home for plugin bundles.

It owns:

- plugin manifests and assets
- plugin-owned example profiles

It does not own:

- core CLI/runtime (`../agent`)
- registry server (`../agent-registry`)

## Status

- plugin bundles were moved here out of `agent`
- this directory is now the package inventory for local installs and registry scanning

## Current focus

Treat each plugin directory as the installable package unit.

Current assumptions:

- package name matches `metadata.name`
- version comes from `plugin.yaml`
- package can be tarballed as-is by the registry

## Current next step

Keep packages registry-friendly while `agent-registry` builds the server and `agent` adds remote search/install.
